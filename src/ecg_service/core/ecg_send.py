...
# def process_pdf(
#     filename: str, csv_path: str, club_name: str, first_name: str, stop_event: Event
# ):
...
#     body = f"""Dear {csv_utils.get_col_from_email("Name",csv_path, email, first_name)},
...
def process_pdf(
    ...
    """Encrypt PDF, upload to S3, and email a secure download link."""
    pdf_path = os.path.join(TEMP_DIR, filename)
    email = os.path.splitext(filename)[0].rsplit("_", 1)[0]
...
Your ECG report is ready. The password has been sent via text to your provided contact number.
...
The CardioLogic Team"""
    ...
    email_utils.send_email(email, "ECG Report - Secure Download Link", body)
    logging.info(f"Email sent to {email}")
...
def process_club_pdfs(
    club_name: str, csv_path: str, first_name: str, stop_event: Event
) -> bool:
